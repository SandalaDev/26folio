...
---
updated: 2026-07-04T23:08:07.065Z
updated_by: claude-code
---
# Current State
## Active work
Epic: EPIC-012   Slice: EPIC-012-SLICE-1   Task: TASK-049
Branch: feature/EPIC-012   Actor: claude-code / claude-fable-5 (planner)
## Completion status
EPIC-012 (design-system fidelity) PLANNED on feature/EPIC-012 off dev (dev holds EPIC-011 via PR #13; repo cleaned: merged local branches EPIC-009/010/011 + chore/os-cleanup-and-simplify deleted; feature/EPIC-008 kept LOCAL-ONLY, it holds an unmerged Resend contact-form spike superseded by TASK-048 but relevant to the future Resend epic). Owner review 2026-07-04: project-spine/10-design-system.html reads better than the shipped site. Gap analysis in backlog/epics/EPIC-012-design-system-fidelity.md: (1) flashlight — preview is global/520px/peach-led/rAF+eased, site is About-only/280px/rose-led/setState-per-mousemove and absent from /capabilities; (2) jagged gradients — hero static fallback runs FULL-strength caramel/rose two-stop ramps (the exact SS3 anti-pattern, bands on dark), shader grain dither is off, Blob washes use 2-8px CSS blur on 26rem-scaled SVGs (stepped edges); (3) tokens under-applied — component grep: rose 42 uses vs soft 1, surface-2 1, success 0, caramel 9 (mostly blob fills), peach 8; the preview distributes caramel labels/soft captions/peach mono/surface-2 elevation. Five tasks, no new tokens, no copy changes, zero functional regression: TASK-049 global DS-spec flashlight (MotionValue-driven), TASK-050 smooth ramps (low-alpha multi-stop hero fallback + MeshBg + shader grain), TASK-051 blob blur proportional to rendered size (SVG-space feGaussianBlur), TASK-052 palette redistribution to SS2 roles, TASK-053 single-weight page h1s + optional ink-to-soft display gradient + sage success state. Two owner open questions recorded in the epic (h2 weight mixes; display gradient). Owner second pass 2026-07-05 added: (4) About flashlight too narrow (folded into TASK-049 — 520px preview recipe is the reference) and (5) featured-work card door-tilt BROKEN — root-caused live: motion values passed conditionally into style after mount are never subscribed by framer-motion (tiltEnabled flips in an effect), DOM transform stays none; latent since EPIC-010, fully exposed when TASK-047 moved the zoom onto the same path. TASK-054 (SLICE-4): fix binding + stable rect measurement + edge-hinged smooth spring, then roll door-tilt out to all cards via a shared TiltCard primitive.
## What is done
...
- EPIC-012 execution: TASK-049 (global DS-spec flashlight) -> TASK-050/051 (gradient smoothness) -> TASK-052/053 (token + type fidelity) -> TASK-054 (card door-tilt fix + all-cards rollout; root cause pre-diagnosed in the task file, do not re-diagnose); epic at backlog/epics/EPIC-012-design-system-fidelity.md
