...
## Reference
The full operating manual lives in `OPERATING_MANUAL.md` — the six-phase
project start, the day-to-day loop, the handoff protocol, the skills layer,
the fail-closed gate, file templates, and the launch checklist. The live Build
Dashboard is `dashboard.html`, generated by `node scripts/render-dashboard.mjs`
(or `bash scripts/os.sh render`). Both are derived; the source of truth is
